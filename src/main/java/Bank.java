...
public class Bank {
    ...
    /**
     * Default constructor that initializes bank name to "Illini Bank".
     */
    public Bank() {
    ...
    public boolean withdrawMoney(final BankAccount bankAccount, final double amount) {
        if (bankAccount == null || amount < 0 || bankAccount.getBalance() < amount) {
            return false;
        }
    ...
    public boolean transferMoney(final BankAccount source, final BankAccount destination,
            final double amount) {
        if (source == null || destination == null || amount < 0 || source.getBalance() < amount) {
            return false;
        } else {
            this.withdrawMoney(source, amount);
            this.depositMoney(destination, amount);
            return true;
        }
    }
    ...
    /**
     * Increment the total account number by 1.
     */
    public static void increseNumberOfAccount() {
        totalAccounts++;
    }
